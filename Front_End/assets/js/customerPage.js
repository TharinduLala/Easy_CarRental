...
$('#editCustomerDetailsBtn').click(function () {
    loadCustomer();
});

$('#saveEditedDetailsBtn').click(function () {
    updateCustomer();
});

$('#closeEditedDetailsBtn').click(function () {
    customerPassword = '';
    customerNicImg = '';
});
...
function bindClickEventsToCarContainer() {
    $('.viewCarDetailsBtn').click(function () {
        carDetailsContainer.children().not('.carDetailsContainerCloseBtn').remove();
        $(this).parent().parent().closest('div').find(carDetailsContainer).css("display", "block");
        let text = $(this).parent().parent().closest('div').find('h3').text();
        let heading = `<h3>${text}</h3>`;
        carDetailsContainer.append(heading);
    });

    $('.carDetailsContainerCloseBtn').click(function () {
        ...
        success: function (res) {
            if (res.statusCode === 200) {
                for (const car of res.data) {
                    appendCar(car.carType, car.carFrontImg, car.carBrand);
                }
            }
        },
        error: function (ob) {
...
function appendCar(type, imgUrl, carBrand) {
    ...
    <div class="d-flex flex-row justify-content-between align-items-center position-relative" style="width: 100%;padding: 0 10px">
        <h3 style="font-size: 20px;line-height: 30px;color: #33334f;margin-bottom: 0;font-family: 'Nunito', sans-serif;font-weight: 600;width: 100%;white-space: nowrap;text-overflow: ellipsis;word-wrap: break-word;">
            ${carBrand}</h3>
    </div>
    ...
    <div aria-label="Basic outlined example" class="btn-group " role="group">
        ...
        <button class="btn btn-primary ps-5 pe-5 " type="button">BOOK NOW</button>
    </div>
    <div class="carDetailsContainer rounded" style="position: absolute;top: 40px;left: 0;right: 0;margin: auto;background: black;color: white;display: none;width: 80%;height: 50%;opacity: 0.8;z-index: 5;">
